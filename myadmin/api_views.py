...
class AdminIndexView(CheckSecurityMixin, CheckAdminPermissionMixin, StatusWrapMixin, JsonResponseMixin, DetailView):
    ...
    def get_queryset(self):
        return {
            'users': FaceUser.objects.all().count(),
...
class AdminChatHistoryView(CheckSecurityMixin, CheckAdminPermissionMixin,
                           StatusWrapMixin, MultipleJsonResponseMixin, ListView):
    http_method_names = ['get']
    model = ChatHistory
    paginate_by = 20
    ...
    def get_queryset(self):
        queryset = super(AdminChatHistoryView, self).get_queryset()
        queryset = queryset.order_by('-chat')
        query_str = self.request.GET.get('query')
        if query_str:
